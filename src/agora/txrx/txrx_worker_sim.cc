...
void TxRxWorkerSim::Start() {
  MLPD_FRAME("TxRxWorkerSim[%zu] starting\n", tid_);
  thread_ = std::thread(&TxRxWorkerSim::DoTxRx, this);
}

void TxRxWorkerSim::Stop() {
  MLPD_FRAME("TxRxWorkerSim[%zu] stopping\n", tid_);
...
void TxRxWorkerSim::SendBeacon(size_t frame_id) {
  ...
  for (size_t beacon_sym = 0u;
    ...
    for (size_t interface = 0u; interface < num_interfaces_; interface++) {
      ...
      for (size_t channel = 0u; channel < Configuration()->NumChannels();
           channel++) {
        const size_t ant_id =
            ((global_interface_id * Configuration()->NumChannels()) + channel);
...
int TxRxWorkerSim::DequeueSend() {
  ...
  const size_t offset =
      (Configuration()->GetTotalDataSymbolIdxDl(frame_id, data_symbol_idx_dl) *
  ...
  udp_clients_.at(ant_id - interface_offset_)
      ->Send(
          Configuration()->BsRruAddr(), Configuration()->BsRruPort() + ant_id,
          reinterpret_cast<uint8_t*>(pkt), Configuration()->DlPacketLength());
