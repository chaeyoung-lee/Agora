...
void TxRxWorkerUsrp::Start() {
  ...
  MLPD_INFO("TxRxWorkerUsrp[%zu] starting\n", tid_);
  thread_ = std::thread(&TxRxWorkerUsrp::DoTxRx, this);

  radioconfig_->Go();
}

void TxRxWorkerUsrp::Stop() {
  MLPD_INFO("TxRxWorkerUsrp[%zu] stopping\n", tid_);
...
void TxRxWorkerUsrp::DoTxRx() {
  ...
  size_t rx_slot = 0;
  //size_t radio_lo = tid * Configuration()->NumRadios() / socket_thread_num_;
  //size_t radio_hi = (tid + 1) * Configuration()->NumRadios() / socket_thread_num_;
